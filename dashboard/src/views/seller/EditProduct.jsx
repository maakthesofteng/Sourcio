...
const EditProduct = () => {
  ...
  const changeImage = (oldImage, event) => {
    const file = event.target.files[0];
    if (!file) return;

    const formData = new FormData();
    formData.append("newImage", file); // Ensure backend expects "file"
    formData.append("oldImage", oldImage);
    formData.append("productId", productId);

    dispatch(product_image_update(formData));
  };
                ...
                imageShow.map((img, i) => (
                  <div>
                    ...
                    <input
                      ...
                      // onChange={(e) =>
                      //   changeImage(product.images?.[0], e.target.files)
                      // }
                      onChange={(e) => changeImage(img, e)}
                      type="file"
                      id={i}
                      className="hidden"
                    />
